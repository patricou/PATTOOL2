...
import { MeteoFranceArpegeTabComponent } from './meteo-france-arpege-tab.component';
...
} from '../shared/weather-point-popup.util';
import {
...
type MeteoFranceMapSource = 'radar' | 'aromepi' | 'forecast';
...
type MeteoFranceMainTab = 'radar' | 'clim' | 'ms-hist' | 'aromepi' | 'arpege' | 'forecast-options' | 'forecast-owm' | 'forecast-om' | 'forecast-ms' | 'ms-precip' | 'forecast-aggregate';
...
@Component({
  ...
  imports: [
    ...
    NgbModule,
    BaseChartDirective,
    MeteoFranceChartPanelComponent,
    WeatherPointTimelineComponent,
    MeteoSwissPrecipTabComponent,
    MeteoFranceArpegeTabComponent,
...
export class MeteoFranceComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(MeteoFranceArpegeTabComponent) arpegeTab?: MeteoFranceArpegeTabComponent;
  ...
  climShowTable = false;
  aromepiShowForecastTable = false;
  owmShowForecastTable = false;
  omShowForecastTable = false;
  msShowForecastTable = false;
  aggregateShowForecastTable = false;
  ...
  private msHistStationIdOverride: string | null = null;
  climFromCache = false;
  msHistFromCache = false;
  isClearingClimHistoryCache = false;
  isClearingMsHistHistoryCache = false;
  optionsHistoryCacheClearLevel: 'ok' | 'error' | '' = '';
  optionsHistoryCacheClearMessageKey = '';
  private optionsHistoryCacheClearTimer: ReturnType<typeof setTimeout> | null = null;
  ...
  forecastCacheMinutes = 5;
  readonly forecastCacheMinMinutes = 1;
  readonly forecastCacheMaxMinutes = 120;
  isClearingForecastCache = false;
  ...
  aromepiDomains: Array<{ id: string; label: string; wmsService?: string }> = [
    { id: '001-FRANCE', label: 'France 0.01°' },
    { id: '0025-FRANCE', label: 'France 0.025°' }
  ];
  aromepiSelectedDomain = '001-FRANCE';
  aromepiSelectedLayer = '';
  aromepiSelectedStyle = '';
  aromepiReferenceTimes: string[] = [];
  aromepiElevations: string[] = [];
  aromepiSelectedElevation = '';
  ...
  ngOnInit(): void {
    ...
    this.loadForecastCachePreferences();
  ...
  private applyNavigationQueryParams(params: ParamMap): boolean {
    ...
    if (tab === 'radar' || tab === 'clim' || tab === 'ms-hist' || tab === 'aromepi' || tab === 'arpege' || tab === 'forecast-owm'
  ...
  private isMfStationLabelPoint(point: TemperatureGridPoint): boolean {
    ...
    if (point.stationId) {
      return this.temperatureLabelSource === 'meteofrance-dpobs';
    }
    return false;
  }
  ...
  private isMsStationLabelPoint(point: TemperatureGridPoint): boolean {
    ...
    if (point.stationId) {
      return this.temperatureLabelSource === 'meteoswiss-smn';
    }
    return false;
  }

  /** MeteoSwiss SMN station (tooltip history, brand) — based on point source/coords, not user location. */
  private isMeteoSwissStationPoint(point: TemperatureGridPoint): boolean {
    if (point.interpolated) {
      return false;
    }
    const source = String(point.source || '').toLowerCase();
    ...
    if (this.temperatureLabelSource === 'meteoswiss-smn' && point.stationId) {
      return true;
    }
    return !!point.stationId
      && this.isCoordinateInSwitzerland(point.lat, point.lon);
  }

  /** Météo-France DPObs station (tooltip history, brand) — based on point source/coords, not user location. */
  private isMeteoFranceStationPoint(point: TemperatureGridPoint): boolean {
    if (point.interpolated) {
      return false;
    }
    const source = String(point.source || '').toLowerCase();
    ...
    if (this.temperatureLabelSource === 'meteofrance-dpobs' && point.stationId) {
      return true;
    }
    return !!point.stationId
      && this.isCoordinateInFranceForMfObs(point.lat, point.lon);
  }

  private canShowMfHistoryInTooltip(point: TemperatureGridPoint): boolean {
    return this.isMeteoFranceStationPoint(point) && this.climAvailable;
  }

  private canShowMsHistoryInTooltip(point: TemperatureGridPoint): boolean {
    return this.isMeteoSwissStationPoint(point) && this.msHistAvailable;
  }

  private resolveStationRefreshProvider(point: TemperatureGridPoint): 'mf' | 'ms' | null {
    if (this.isMeteoSwissStationPoint(point)) {
      return 'ms';
    }
    if (this.isMeteoFranceStationPoint(point)) {
      return 'mf';
    }
    return null;
  }

  private buildTemperatureRefreshButtonHtml(point: TemperatureGridPoint, key: string, refreshing: boolean): string {
    const provider = this.resolveStationRefreshProvider(point);
    if (!provider) {
      return '';
    }
    ...
    const label = refreshing
      ...
      : this.escapeHtml(this.translate.instant(labelKey));
    const logo = this.buildBrandLogoHtml(brand, 12);
    const title = this.escapeHtml(this.getBrandAlt(brand));
    return (
  ...
  get aromepiStatusMessageKey(): string {
    ...
    if (this.mfStatus.aromepiAuthError && String(this.mfStatus.aromepiAuthError).toLowerCase().includes('truncated')) {
      return 'METEO_FRANCE.AROMEPI_TOKEN_TRUNCATED';
    }
  ...
  get showAromepiElevationControl(): boolean {
    return this.aromepiElevations.length > 0;
  }
  ...
  get showMfArpegeTab(): boolean {
    return this.isLocationInFrance();
  }

  get arpegeAvailable(): boolean {
    return this.mfStatus?.arpegeAuthValid === true;
  }

  get arpegeStatusMessageKey(): string {
    if (!this.mfStatus) {
      return '';
    }
    if (!this.mfStatus.arpegeConfigured) {
      return 'METEO_FRANCE.ARPEGE_NOT_CONFIGURED';
    }
    if (this.mfStatus.arpegeAuthError && String(this.mfStatus.arpegeAuthError).toLowerCase().includes('truncated')) {
      return 'METEO_FRANCE.ARPEGE_TOKEN_TRUNCATED';
    }
    if (this.mfStatus.arpegeAuthValid === false) {
  ...
  get showMsHistTab(): boolean {
    return this.isLocationInSwitzerland() || !!this.msHistStationIdOverride;
  }

  get msHistAvailable(): boolean {
    return this.mfStatus?.meteoswissSmnEnabled !== false;
  }
  ...
  private loadForecastCachePreferences(): void {
    this.subs.add(
      this.apiService.getMeteoFranceForecastCachePreferences().subscribe({
        next: (pref) => {
          if (pref?.forecastCacheMinutes != null) {
  ...
  onForecastCacheMinutesCommitted(): void {
    if (!this.guardMeteoOptionsEdit()) {
      return;
    }
    const n = Math.round(Number(this.forecastCacheMinutes));
    this.forecastCacheMinutes = Number.isFinite(n)
      ? Math.max(this.forecastCacheMinMinutes, Math.min(this.forecastCacheMaxMinutes, n))
      : 5;
    this.subs.add(
  ...
  onClearForecastCaches(): void {
    if (this.isClearingForecastCache) {
      return;
    }
    this.isClearingForecastCache = true;
    this.subs.add(
      this.apiService.clearMeteoFranceForecastCaches().subscribe({
        next: () => {
          this.isClearingForecastCache = false;
          ...
          if (this.aromepiAvailable) {
            ...
            this.loadAromepiCapabilities({ force: true, keepSelectedLayer: true });
          }
          this.arpegeTab?.reloadAfterForecastCacheClear();
        },
        error: () => {
          this.isClearingForecastCache = false;
          this.showForecastCacheClearToast('METEO_FRANCE.FORECAST_CACHE_CLEAR_ERR', 'error');
        }
      })
    );
  }

  private showForecastCacheClearToast(messageKey: string, level: 'ok' | 'error'): void {
    this.optionsForecastCacheClearMessageKey = messageKey;
    this.optionsForecastCacheClearLevel = level;
    if (this.optionsForecastCacheClearToastTimer) {
      clearTimeout(this.optionsForecastCacheClearToastTimer);
    }
    this.optionsForecastCacheClearToastTimer = setTimeout(() => {
      this.optionsForecastCacheClearMessageKey = '';
      this.optionsForecastCacheClearLevel = '';
      this.optionsForecastCacheClearToastTimer = null;
    }, 4000);
  }
  ...
  private formatSelectedMfStationProximityLabel(): string {
    return formatMfStationProximityLabel(
      this.formatSelectedMfStationLabel(),
      this.selectedPointMfStation?.id ?? '',
      this.selectedPointMfStationDistKm,
      (station, km) => this.translate.instant('METEO_FRANCE.POINT_SOURCE_MF_NEAREST', { station, km })
    );
  }
  ...
  private formatSelectedMsStationProximityLabel(): string {
    return formatMfStationProximityLabel(
      this.formatSelectedMsStationLabel(),
      this.selectedPointMsStation?.id ?? '',
      this.selectedPointMsStationDistKm,
      (station, km) => this.translate.instant('METEO_FRANCE.POINT_SOURCE_MS_NEAREST', { station, km })
    );
  }
  ...
  private updateSelectedPointMfStationDistance(
    ...
    this.selectedPointMfStationDistKm = resolveMfStationDistanceKm(
      clickLat,
      clickLon,
      mfPoint,
      this.selectedPointMfStation?.id,
      this.temperatureGridPoints,
      (lat, lon) => this.findNearestGridPoint(lat, lon)
    );
  }
  ...
  private updateSelectedPointMsStationDistance(
    ...
    this.selectedPointMsStationDistKm = resolveMfStationDistanceKm(
      clickLat,
      clickLon,
      msPoint,
      this.selectedPointMsStation?.id,
  ...
  get pointTempTimelineAltitudeM(): number | null {
    const point = this.findNearestGridPoint(this.lat, this.lon);
    const alt = point?.altitudeM;
    return alt != null && Number.isFinite(alt) ? alt : null;
  }
  ...
  private resolveSelectedLocationName(): string {
    return resolveWeatherPointLocationLabel({
      geocodeName: this.selectedLocationName,
      openWeatherPlace: this.selectedPointOpenWeatherPlace,
      city: this.city,
      lat: this.lat,
      lon: this.lon,
    });
  ...
  private isTemperatureMapUiClick(event: Event | undefined): boolean {
    ...
    return !!target?.closest('.mf-temp-tooltip, .mf-temp-label, .mf-temp-refresh-btn, .ms-temp-refresh-btn, .mf-temp-history-btn, .ms-temp-history-btn, .mf-selected-point-weather-overlay, .mf-point-timeline-card');
  }
  ...
  private readonly onTemperatureTooltipClick = (event: MouseEvent): void => {
    ...
    if (msHistoryButton) {
      ...
      if (historyKey) {
        this.openMeteoSwissStationHistory(historyKey);
      }
      return;
    }
  ...
  private openMeteoSwissStationHistory(key: string): void {
    const point = this.temperatureGridPoints.find((p) => this.temperaturePointKey(p) === key);
    if (!point?.stationId || !this.isMeteoSwissStationPoint(point) || !this.msHistAvailable) {
      return;
    }
    ...
    this.msHistStationIdOverride = stationId || null;
    // Use station coordinates for the history query without setLocation (which clears countryCode).
    this.lat = stationLat;
    this.lon = stationLon;
  ...
  private switchToMsHistTab(): void {
    const tab = this.resolveRegionalMainTab('ms-hist');
    if (tab !== 'ms-hist') {
      return;
    }
    this.stopAromepiAnimation();
    this.prepareMsHistTabLoad();
    this.activeMainTab = 'ms-hist';
  }
  ...
  private upsertMfTemperatureGridPoint(rawPoint: any): TemperatureGridPoint | null {
    ...
    if (!this.isMeteoFranceStationPoint(normalized)) {
      return null;
    }
  ...
  private upsertMsTemperatureGridPoint(rawPoint: any): TemperatureGridPoint | null {
    const normalized = this.normalizeTemperatureGridPoint(rawPoint, false);
    normalized.cached = false;
    if (!this.isMeteoSwissStationPoint(normalized)) {
      return null;
    }
    const key = this.temperaturePointKey(normalized);
    const index = this.temperatureGridPoints.findIndex((p) => this.temperaturePointKey(p) === key);
  ...
  private buildTemperatureLabelTooltip(point: TemperatureGridPoint): string {
    ...
    if (point.stationId && this.shouldShowStationLabels()) {
      ...
      let actions =
        ...
        this.buildTemperatureRefreshButtonHtml(point, key, refreshing);
      if (this.canShowMfHistoryInTooltip(point)) {
      ...
      if (this.canShowMsHistoryInTooltip(point)) {
        const historyLabel = this.escapeHtml(this.translate.instant('METEO_FRANCE.TEMPERATURE_TOOLTIP_HISTORY'));
        actions +=
  ...
  private isObsStationPoint(point: TemperatureGridPoint): boolean {
    ...
    return this.isMeteoFranceStationPoint(point) || this.isMeteoSwissStationPoint(point);
  }
  ...
  /** Open Trace Viewer with the same center, zoom and basemap as the visible map. */
  openCurrentMapInTraceViewer(source: MeteoFranceMapSource = 'radar'): void {
    if (!this.traceViewerModalComponent) {
      return;
    }
    const map =
      source === 'aromepi' ? this.aromepiMap
        : source === 'forecast' ? this.forecastMap
          : this.map;
    let lat: number;
    let lon: number;
    let zoom: number;
    if (map) {
      ...
      this.ensureSharedMapViewDefaults();
      lat = this.sharedMapCenterLat!;
      lon = this.sharedMapCenterLon!;
  ...
  private resolveRegionalMainTab(tab: MeteoFranceMainTab): MeteoFranceMainTab {
    ...
    if (tab === 'arpege' && !this.showMfArpegeTab) {
      return this.showMeteoSwissForecastTabs ? 'forecast-ms' : 'radar';
    }
  ...
  loadAromepiCapabilities(options?: { force?: boolean; keepSelectedLayer?: boolean }): void {
    if (!this.aromepiAvailable) {
      return;
    }
    const force = !!options?.force;
    const keepSelectedLayer = !!options?.keepSelectedLayer;
    if (this.isLoadingAromepiCapabilities || (!force && this.aromepiCapabilities)) {
      return;
    }
    ...
    const refForRequest = keepSelectedLayer ? this.aromepiReferenceTime : undefined;
    this.subs.add(
      this.apiService.getMeteoFranceAromepiCapabilities(this.aromepiSelectedDomain, refForRequest).subscribe({
        next: (caps) => {
          ...
          if (caps?.cacheTtlMinutes != null && Number.isFinite(Number(caps.cacheTtlMinutes))) {
            this.aromepiForecastCacheTtlMinutes = Number(caps.cacheTtlMinutes);
          }
          if (Array.isArray(caps?.domains) && caps.domains.length) {
            this.aromepiDomains = caps.domains;
          }
          if (caps?.domain) {
            this.aromepiSelectedDomain = String(caps.domain);
          }
          ...
          this.aromepiReferenceTimes = Array.isArray(caps.referenceTimes) ? caps.referenceTimes : [];
          this.aromepiReferenceTime =
            (refForRequest && this.aromepiReferenceTimes.includes(refForRequest) ? refForRequest : null)
            || caps.defaultReferenceTime
            || this.aromepiReferenceTimes[this.aromepiReferenceTimes.length - 1]
            || '';
          const globalElevations = Array.isArray(caps?.elevations) ? caps.elevations.map(String) : [];
          if (!keepSelectedLayer || !this.aromepiSelectedLayer
              || !this.aromepiLayers.some((l) => l.name === this.aromepiSelectedLayer)) {
            if (this.aromepiLayers.length) {
              const cloud = this.aromepiLayers.find((l) => l.category === 'cloud');
              const precip = this.aromepiLayers.find((l) => l.category === 'precipitation');
              const pick = cloud || precip || this.aromepiLayers[0];
              this.aromepiSelectedLayer = pick.name;
              this.aromepiSelectedStyle = pick.style || '';
            } else {
              this.aromepiSelectedLayer = '';
              this.aromepiSelectedStyle = '';
            }
          }
          const selected = this.aromepiLayers.find((l) => l.name === this.aromepiSelectedLayer);
          this.syncAromepiElevationsForLayer(selected, globalElevations, caps?.defaultElevation);
          this.aromepiFrameIndex = this.findAromepiFrameIndexClosestToNow();
          this.clampAromepiFrameIndex();
          this.logBackend(
          ...
          this.clearAromepiPrefetchLayers();
          this.setupAromepiWmsLayer(false, () => {
  ...
  onAromepiDomainChange(domainId: string): void {
    if (!domainId || domainId === this.aromepiSelectedDomain) {
      return;
    }
    this.aromepiSelectedDomain = domainId;
    this.aromepiSelectedLayer = '';
    this.aromepiSelectedElevation = '';
    this.aromepiCapabilities = null;
    this.loadAromepiCapabilities({ force: true });
  }

  onAromepiReferenceRunChange(runIso: string): void {
    if (!runIso || runIso === this.aromepiReferenceTime) {
      return;
    }
    this.aromepiReferenceTime = runIso;
    this.loadAromepiCapabilities({ force: true, keepSelectedLayer: true });
  }

  onAromepiElevationChange(elevation: string): void {
    this.aromepiSelectedElevation = elevation || '';
    this.clearAromepiPrefetchLayers();
  ...
  onAromepiLayerChange(layerName: string): void {
    ...
    this.syncAromepiElevationsForLayer(layer);
    this.clearAromepiPrefetchLayers();
    this.setupAromepiWmsLayer();
    this.loadAromepiCurrentValues();
  }

  private syncAromepiElevationsForLayer(
    layer?: { elevations?: string[]; defaultElevation?: string },
    globalElevations: string[] = [],
    globalDefault?: string
  ): void {
    const layerElevations = Array.isArray(layer?.elevations) ? layer!.elevations!.map(String) : [];
    this.aromepiElevations = layerElevations.length ? layerElevations : globalElevations;
    if (!this.aromepiElevations.length) {
      this.aromepiSelectedElevation = '';
      return;
    }
    const preferred = layer?.defaultElevation || globalDefault || this.aromepiSelectedElevation;
    this.aromepiSelectedElevation = preferred && this.aromepiElevations.includes(String(preferred))
      ? String(preferred)
      : this.aromepiElevations[0];
  }
    ...
    layers.forEach((layer) => {
      this.subs.add(
        this.apiService.getMeteoFranceAromepiFeatureInfo(
          ...
          layer.style,
          this.aromepiSelectedDomain,
          this.aromepiSelectedElevation || undefined
        ).subscribe({
          next: (res) => {
            if (!res?.error) {
  ...
  private buildAromepiWmsUrl(layer: string, style: string, time: string, referenceTime: string): string {
    return this.apiService.buildMeteoFranceAromepiWmsTileUrl(
      '{z}', '{x}', '{y}',
      layer,
      time,
      referenceTime,
      ...
      this.aromepiSelectedDomain,
      this.aromepiSelectedElevation || undefined,
      this.aromepiWmsCacheBust > 0 ? this.aromepiWmsCacheBust : undefined
    );
  }

  private createAromepiWmsTileLayer(url: string, opacity: number, trackCache = true): L.TileLayer {
  ...
  /**
   * Leaflet TileLayer that loads via fetch so we can read X-Pat-Cache (HIT/MISS)
   * for the Live/Cache badge. Object URLs are revoked on tileunload.
   */
  ...
  private applyAromepiTileCacheStatsFromLayer(layer: L.TileLayer | null | undefined): void {
    const stats = (layer as L.TileLayer & { _patCacheStats?: { hits: number; misses: number } } | null)?._patCacheStats;
  ...
  private applyAromepiThrottleFromStatus(status: {
    ...
    if (status?.forecastCacheTtlMinutes != null && Number.isFinite(Number(status.forecastCacheTtlMinutes))) {
      this.aromepiForecastCacheTtlMinutes = Number(status.forecastCacheTtlMinutes);
    }
  ...
  private scheduleAromepiCacheStatusProbe(): void {
    // Kept for call sites that still schedule after errors; badge is driven by tile fetch headers.
    if (this.aromepiCacheProbeTimer) {
      clearTimeout(this.aromepiCacheProbeTimer);
    }
    this.aromepiCacheProbeTimer = this.scheduleComponentTimeout(() => {
      this.aromepiCacheProbeTimer = null;
      this.probeAromepiThrottleStatus();
    }, 500);
  }
  ...
  private prefetchAromepiFrame(frameIndex: number): void {
    ...
    const layer = this.createAromepiWmsTileLayer(url, 0, false);
  ...
  private applyGeocodeMetadata(res: any, lat: number, lon: number): void {
    ...
    this.selectedLocationName = resolvePlaceNameFromGeocode(res, lat, lon);
    const cityName = extractGeocodeCityName(res);
    if (cityName) {
      this.city = cityName;
    }
    ...
    this.subs.add(
      this.apiService.clearMeteoFranceClimCache().subscribe({
        next: () => {
          ...
          this.showOptionsHistoryCacheClearMessage('ok', 'METEO_FRANCE.HISTORY_CACHE_CLEAR_OK_MF');
          this.climFromCache = false;
          this.loadClimData(this.climSelectedStationId || undefined, true);
        },
        error: () => {
          this.isClearingClimHistoryCache = false;
          this.showOptionsHistoryCacheClearMessage('error', 'METEO_FRANCE.HISTORY_CACHE_CLEAR_ERR_MF');
          this.loadClimData(this.climSelectedStationId || undefined, true);
        }
      })
    );
  ...
  private currentMsHistCacheKey(): string {
    return this.historyCache.msHistKey(this.lat, this.lon, this.msHistDays, this.msHistStationIdOverride || undefined);
  }
  ...
  private syncMsHistFromCacheFlag(): void {
    this.msHistFromCache = !!this.msHistData?.cached
      || this.historyCache.lookupMsHist({
        ...
        stationId: this.msHistStationIdOverride || undefined
      }) != null;
  }
  ...
  loadMsHistData(force = false, stationId?: string): void {
    if (!this.showMsHistTab) {
      this.msHistData = null;
      return;
    }
    if (stationId !== undefined) {
      this.msHistStationIdOverride = stationId ? String(stationId).trim() : null;
    }
    const resolvedStationId = this.msHistStationIdOverride || undefined;
    const msQuery = { lat: this.lat, lon: this.lon, days: this.msHistDays, stationId: resolvedStationId };
    ...
    this.subs.add(
      this.apiService.getMeteoSwissHistoryNearby(this.lat, this.lon, this.msHistDays, resolvedStationId, force).subscribe({
        next: (data) => {
          this.isLoadingMsHist = false;
    ...
    this.subs.add(
      this.apiService.clearMeteoSwissHistoryCache().subscribe({
        next: () => {
          ...
          this.showOptionsHistoryCacheClearMessage('ok', 'METEO_FRANCE.HISTORY_CACHE_CLEAR_OK_MS');
          this.msHistFromCache = false;
          this.loadMsHistData(true);
        },
        error: () => {
          this.isClearingMsHistHistoryCache = false;
          this.showOptionsHistoryCacheClearMessage('error', 'METEO_FRANCE.HISTORY_CACHE_CLEAR_ERR_MS');
          this.loadMsHistData(true);
        }
      })
    );
  ...
  private showOptionsHistoryCacheClearMessage(
    level: 'ok' | 'error',
    messageKey: string
  ): void {
    this.optionsHistoryCacheClearLevel = level;
    this.optionsHistoryCacheClearMessageKey = messageKey;
    if (this.optionsHistoryCacheClearTimer) {
      clearTimeout(this.optionsHistoryCacheClearTimer);
    }
    this.optionsHistoryCacheClearTimer = setTimeout(() => {
      this.optionsHistoryCacheClearLevel = '';
      this.optionsHistoryCacheClearMessageKey = '';
      this.optionsHistoryCacheClearTimer = null;
      this.cdr.markForCheck();
    }, 4500);
  }
  ...
  loadStationHistoryData(forceRefresh = false): void {
    ...
    if (!this.climAvailable || !point?.stationId || !this.stationHistoryModalVisible
        || !this.isMeteoFranceStationPoint(point)) {
      return;
    }
