...
/**
 * Bandeau d'actualités défilant, par username, dans {@code appParameters}
 * sous {@code news.ticker.&lt;username&gt;}. Défaut applicatif : désactivé.
 */
@Service
public class NewsTickerPreferenceService {

    private static final Logger log = LoggerFactory.getLogger(NewsTickerPreferenceService.class);

    static final String PARAM_KEY_PREFIX = "news.ticker.";

    private final AppParameterService appParameterService;
    private final ObjectMapper objectMapper;
    private final UserOwnerService userOwnerService;

    public NewsTickerPreferenceService(
    ...
    public Optional<Boolean> findForSubject(String jwtSubject) {
        ...
        try {
            NewsTickerDto dto = objectMapper.readValue(raw, NewsTickerDto.class);
            ...
            log.debug("news.ticker illisible: {}", e.getMessage());
            return Optional.empty();
        }
    }
    ...
    public boolean saveForSubject(String jwtSubject, boolean enabled) {
        ...
        try {
            String json = objectMapper.writeValueAsString(new NewsTickerDto(enabled));
            appParameterService.setJson(
                    key,
                    json,
                    "Actualités : bandeau défilant choisi par l'utilisateur (JSON).");
        } catch (JsonProcessingException e) {
            throw new IllegalStateException("Serialization news ticker", e);
        }
