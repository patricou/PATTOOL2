...
/**
 * Resolves which members can see a given {@link Note}. Logic mirrors
 * {@link TodoListVisibilityService}.
 */
@Service
public class NoteVisibilityService {
    ...
    public List<CalendarVisibilityRecipientDTO> listVisibilityRecipients(Note note) {
        Set<String> ids = resolveRecipientMemberIds(note);
    ...
    public Set<String> resolveRecipientMemberIds(Note note) {
        Set<String> ids = new HashSet<>();
        String ownerId = note.getOwnerMemberId();
        ...
        if ("friendGroups".equals(vis)) {
            if (note.getFriendGroupIds() != null) {
                for (String gid : note.getFriendGroupIds()) {
                    if (StringUtils.hasText(gid)) {
                        ids.addAll(memberIdsWithAccessToFriendGroup(gid.trim()));
                    }
                }
            }
            if (StringUtils.hasText(note.getFriendGroupId())) {
                ids.addAll(memberIdsWithAccessToFriendGroup(note.getFriendGroupId().trim()));
            }
            return ids;
        }
        if (StringUtils.hasText(note.getFriendGroupId())) {
